...
function executerRequete(evt) {
    ...
    const texteRequete = formulaireValidé.querySelector('[name=requete]').value;
    console.log("\t J'ai récupéré ce texte de requête : " + texteRequete);

    let adresseDesDonnees;
    if (BDLocale) {
        adresseDesDonnees = "php/soumettre-requete/controller.php";
    } else {
        adresseDesDonnees = "https://web2noninfo.lpmiaw.univ-lr.fr/ISI/projet/api/soumettre-requete/controller.php";
    }
    ...
    if (!erreurDeBD) {
        ...
        afficherErreur("Le choix entre la base de données prof et votre base de donnée n'est pas clairement exprimé");
    }
...
function afficherResultatsRequete(enregistrements) {
    ...
    {
        afficherErreur("Problème de communication Ajax");
    } else {
